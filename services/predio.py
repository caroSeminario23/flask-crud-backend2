...
def insert():
    ...
    if not id_tipo_predio or not descripcion or not ruc or not telefono or not correo or not direccion or not idubigeo or not id_persona or not url_imagen or not total_mdu: # Si no se obtiene alguno de los valores solicitados
        result["status_code"]=400
        result["msg"]="Faltan datos"
        return jsonify(result), 400
    
    predio = Predio(id_tipo_predio, descripcion, ruc, telefono, correo, direccion, idubigeo, id_persona, url_imagen, total_mdu) # Se crea una instancia de la clase Contact con los valores obtenidos
    ...
    result['msg']="Faltan datos" # Indica el mensaje de la solicitud
